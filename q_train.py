...
def main(args):
    ...
    args.run_name = args.run_name.replace("Seer",
                                          f"Seer_ptbs{ptbs}_lr{args.learning_rate}_no-tanh_log-clamp")
    ...
    optimizer = torch.optim.AdamW([p for p in ddp_model.parameters() if p.requires_grad], lr=args.learning_rate,
                                  weight_decay=args.weight_decay)  # TODO make sure the parameters which need to be optimized are passing
